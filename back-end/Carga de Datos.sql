...
CREATE OR REPLACE PROCEDURE Ingresar_Prediccion (pre_local in INTEGER, pre_visitante in INTEGER, cli in VARCHAR2, even in INTEGER, obtenido in INTEGER)
AS
BEGIN
        INSERT INTO PREDICCION (PUNTOD_LOCAL, PUNTOS_VISITANTE,ID_CLIENTE, ID_EVENTO, PUNTOS_OBTENIDOS)
        VALUES (pre_local, 
        pre_visitante,
...
SELECT CLIENTE.USUARIO, COUNT(PREDICCION.PUNTOS_OBTENIDOS) FROM CLIENTE, PREDICCION WHERE CLIENTE.ID = PREDICCION.ID_CLIENTE and PREDICCION.PUNTOS_OBTENIDOS = '10'
GROUP BY CLIENTE.USUARIO
ORDER BY CLIENTE.USUARIO

SELECT CLIENTE.USUARIO, COUNT(PREDICCION.PUNTOS_OBTENIDOS) FROM CLIENTE, PREDICCION WHERE CLIENTE.ID = PREDICCION.ID_CLIENTE and PREDICCION.PUNTOS_OBTENIDOS = '5'
GROUP BY CLIENTE.USUARIO
ORDER BY CLIENTE.USUARIO 

SELECT CLIENTE.USUARIO, COUNT(PREDICCION.PUNTOS_OBTENIDOS) FROM CLIENTE, PREDICCION WHERE CLIENTE.ID = PREDICCION.ID_CLIENTE and PREDICCION.PUNTOS_OBTENIDOS = '3'
GROUP BY CLIENTE.USUARIO
ORDER BY CLIENTE.USUARIO 

SELECT CLIENTE.USUARIO, COUNT(PREDICCION.PUNTOS_OBTENIDOS) FROM CLIENTE, PREDICCION WHERE CLIENTE.ID = PREDICCION.ID_CLIENTE and PREDICCION.PUNTOS_OBTENIDOS = '0'
GROUP BY CLIENTE.USUARIO
ORDER BY CLIENTE.USUARIO
...
SELECT * FROM (SELECT ID, TO_CHAR(JORNADA.FECHA_INICIO,'YYYY-MM-DD HH24:MI') FROM JORNADA --WHERE JORNADA.ID_TEMPORADA = 3
ORDER BY TO_CHAR(JORNADA.FECHA_FIN,'YYYY-MM-DD HH24:MI') DESC)
WHERE ROWNUM = 1
